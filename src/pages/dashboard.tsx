...
import { getGrenades } from 'apiSdk/grenades';
import { GrenadeInterface } from 'interfaces/grenade';
import { getWeapons } from 'apiSdk/weapons';
import { WeaponInterface } from 'interfaces/weapon';
import { getMissions } from 'apiSdk/missions';
import { MissionInterface } from 'interfaces/mission';

import { WeaponListPage as ShowCaseList } from 'pages/weapons';

function DashboardPage() {
  const { session } = useSession();

  const grenadeStat = useSWR<PaginatedInterface<GrenadeInterface>>(
    () => `/grenades`,
    () => getGrenades({ limit: 1, offset: 0 }),
  );

  const weaponStat = useSWR<PaginatedInterface<WeaponInterface>>(
    () => `/weapons`,
    () => getWeapons({ limit: 1, offset: 0 }),
  );

  const missionStat = useSWR<PaginatedInterface<MissionInterface>>(
    () => `/missions`,
    () => getMissions({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; isLoading: boolean }[] = [
    {
      title: 'Grenades',
      result: grenadeStat?.data?.totalCount,
      isLoading: grenadeStat.isLoading,
    },
    {
      title: 'Weapons',
      result: weaponStat?.data?.totalCount,
      isLoading: weaponStat.isLoading,
    },
    {
      title: 'Missions',
      result: missionStat?.data?.totalCount,
      isLoading: missionStat.isLoading,
    },
  ];
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`weapons`} passHref legacyBehavior>
                  <Button
