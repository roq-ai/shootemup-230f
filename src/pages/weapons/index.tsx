...
import { getWeapons, deleteWeaponById } from 'apiSdk/weapons';
import { WeaponInterface } from 'interfaces/weapon';

type ColumnType = ColumnDef<WeaponInterface, unknown>;
...
export function WeaponListPage(props: WeaponListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<WeaponInterface>>(
    () => `/weapons?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: WeaponInterface) => {
    if (hasAccess('weapon', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/weapons/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/weapons/view/${record.id}`} passHref legacyBehavior>
                <Button
                  onClick={(e) => e.stopPropagation()}
                  mr={2}
                  padding="0rem 8px"
              ...
              {hasAccess('weapon', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/weapons/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('weapon', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="weapon" />
        </Flex>

        {hasAccess('weapon', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/weapons/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
